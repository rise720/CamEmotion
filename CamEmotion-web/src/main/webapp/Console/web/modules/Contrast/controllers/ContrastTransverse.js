...
//    	}
    ...
    methods:{
        ...
        btnClick: function(items) {
            ...
            this.searchData.currentPage = items;    
            this.queryTerm();
        ...
        preClick: function() {
            this.cur--;
            this.searchData.currentPage = this.cur;  
            this.queryTerm();
        },
        /**
		 * @下一页
    	 ...
    	 * 
    	 */
    	showSetParams:function(){
    		var  that = this;
    		that.error.errorStatus =  false;
    		$('#setupParams').modal('show');
...
//								that.schoolList = data.data;
        ...
        queryTerm:function(){
        	...
        	//添加
        	that.searchData.idList = [];
            if (that.curriculums.length > 0) {
				var idList = [];
				that.curriculums.forEach(function(val, index){
					idList.push(val.id);
				});
				that.searchData.idList = idList;
			}
            var params = that.searchData;
            
            try {
            	requsestSer.post(camEmotion.getCurriculumList, params).then(function (data) {
                	var handleCulData;
                	$('#com-loading').modal('hide');
                	if (data.meta.success) {
                		if(data.data.objList != null && data.data.objList.length > 0){
                			that.error.errorStatus =  false;
                			if(data.data.totalRows == 1){
                				if(that.curriculums.length == 0){
                	    			that.curriculums.push(data.data.objList[0]);
                	    		}else{
                	    			var isExist = false;
                	    			that.curriculums.forEach(function(val, index){
                    					if(val.id == data.data.objList[0].id){
                    						bootAlert.alertDanger("满足条件的课堂已存在，请重新选择");
                    						isExist = true;
                    					}
                    				});
                	    			if(!isExist){
                			...
                			that.error.errorMsg = "没有满足条件的数据";
                		}
                    } else {
                         that.error.errorStatus =  true;
                         that.error.errorMsg = "获取数据异常，请重试";
                    }
                },function (response) {
                   $('#com-loading').modal('hide');
                   that.error.errorStatus =  true;
    	 ...
    	 * 获取选中的课堂 TODO
    	 */
    	getCurriculum:function(curriculum){
    		var that = this;
    		//判断
    		if(that.curriculums.length == 0){
    		...
    		$('#selectResult').modal('hide');
    	},
    	delectQueryData:function(curriculum){
    		var that = this;
    		var that = this;
        	Modal.alert({
                msg: '内容',
            ...
            Modal.confirm({msg: "是否删除？"})
                .on(function (e) {
                    if (e) {
                    	that.curriculums.forEach(function(val, index){
                			if(val.id == curriculum.id){
                				that.curriculums.splice(index,1);
                			}
                		});
                    }
                });
...
//    			//波动率
//    			val.valenceVolatilityList.forEach(function(value,i){
//    				data4[index] = value.percent;
//    			});
//    			//波动偏差
//    			val.fluctuationDeviationList.forEach(function(value,i){
//    				data5[index] = value.percent;
//    			});
//    			//波动均衡
//    			val.uniformFluctuationList.forEach(function(value,i){
//    				data6[index] = value.percent;
//    			});
    		});
	    ...
	    LineChart3:function(lineData, dom, Type){
	    	...
	    	lineData[0].forEach(function(val, index){
	    		xLineData[index] = val.teacherName + "\n" + val.courseContents;
	    	});
	    	if(Type == 'activity'){
	    	...
	    	var option = {
	                ...
	                ,formatter:function(data){
	                	...
	                	for (var int = 0; int < data.length; int++) {
	                		...
	                		+ data[int].marker + "课程内容：" + lineData[0][data[int].dataIndex].courseContents + "</br>"
	                		+ data[int].marker + "" + data[int].seriesName + "：" + data[int].value + "%</br>";
	                	}
